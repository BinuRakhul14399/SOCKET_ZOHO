...
{  
    int mode;
    printf("Which sys files you want to view \t 1.HOST  2.VIRTUAL OS\n");
    scanf("%d",&mode);
    switch (mode)
    {
 ...
 char data[100];
  ...
  if (setsockopt(server_sockfd, SOL_SOCKET,SO_REUSEADDR, &(char){ 1 }, sizeof(int)) < 0)
  {
  ...
  if(listen(server_sockfd, 5) < 0) {
	...
	send(client_sockfd,dirPath,100,0);
    struct dirent *de;

    while(recv(client_sockfd,data, 100, 0)==100)
    {
...
{
	int modeModify;
    printf("Which sys files you want to modify \t 1.HOST  2.VIRTUAL OS\n");
    scanf("%d",&modeModify);
    switch (modeModify)
    {
...
SOCKET server_sockfd, client_sockfd;
 ...
 int siNo=0,count=0;
  ...
  send(client_sockfd,dirPath,100,0);
  send(client_sockfd,&option,sizeof(option),0); 
  char data[100];
  

	while(recv(client_sockfd,data, 100, 0)>0)
    {
    ...
    siNo++;
     ...
     printf("Enter the serial number of the file :");
		scanf("%d",&selectNo);
		send(client_sockfd,&selectNo,sizeof(selectNo),0);
		printf("\nThe filename to be renamed can be provided in client menu--->\n");
	sleep(8);
